...
import { listDocumentAuditEvents } from '../services/document-audit.js';
...
export const handler: APIGatewayProxyHandler = async (event): Promise<APIGatewayProxyResult> => {
  ...
  try {
    const items = await listDocumentAuditEvents(documentId, principal, {
      dynamodb,
    ...
    console.error('listDocumentAuditEvents failed', {
      requestId,
      documentId,
      errorName: err instanceof Error ? err.name : 'UnknownError',
    });
    return errorResponse(500, {
      code: 'LIST_DOCUMENT_AUDIT_EVENTS_FAILED',
      message: 'Không thể tải lịch sử hoạt động. Vui lòng thử lại.',
      requestId,
    });
  }
};
